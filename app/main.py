...
class HouseFeatures(BaseModel):
    ...
    HouseAge: float = Field(
        ge=0, description="Median age of houses in the neighbourhood"
    )
    AveRooms: float = Field(gt=0, description="Average number of rooms per dwelling")
    AveBedrms: float = Field(
        gt=0, description="Average number of bedrooms per dwelling"
    )
...
async def predict_file(file: UploadFile = File(...)):
    ...
    if missing_columns:
        raise HTTPException(
            status_code=400,
            detail=f"Thease columns are missing from your file{missing_columns}",
        )

    if len(df) == 0:
        raise HTTPException(
            status_code=400, detail="The uploaded file has no data rows"
        )

    try:
        df["prediction"] = model.predict(df[required_columns])

        df["predicted_price_usd"] = (df["prediction"] * 100000).apply(
            lambda x: f"${x:,.0f}"
        )
